...
- 分页。
- 开户入口应靠近筛选区域或空状态区域。
- 账号卡片/列表字段固定为 9 项：账号类型（Demo/Live）、平台/服务器（平台为 MT4/MT5/Sirix/XOH/Fortex，服务器为交易账号所在服务器）、账号、余额、净值、信用金、账户类型、杠杆、保证金比例。
...
- 平台/服务器可以合并显示，也可以拆成平台、服务器两列，但同一个模块内只能使用一种结构。
