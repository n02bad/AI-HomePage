...
- 组件库参考必须先过评分准入：只允许参考 5 分以上积木；5 分及以下积木不得进入模型上下文、兜底组件、`componentReferences`、`brickPlan.referenceComponentId` 或审美样本。
- 最低保底是“高分积木同款微调”：如果 AI 无法稳定生成更好的组件或页面，就直接采用同家族/同尺寸最高匹配高分积木的主结构，只微调文案、主题 token、系统动作、响应式和尺寸适配。
- 推荐生成是“高分积木上升级”：保留高分积木的业务字段、按钮层级、状态标签、卡片密度和图表/列表表达，再提升首屏规划、模块比例、排版节奏、信息层级和响应式组合。
- 组件库参考只提供形态、语汇和灵感，不代表可以新增业务功能；推荐输出不要直接复制某个积木或只换颜色，要在布局、密度、层级或组合方式上形成新的合理发散。只有兜底路径允许近似同款复用。
...
- `1x*` 组件进入首页时占 `4/12` 栏，适合 rail、小卡、辅助入口。
- `2x*` 组件进入首页时占 `8/12` 栏，适合主内容、横卡、组合信息区。
- `3x*` 及以上组件进入首页时占 `12/12` 栏，适合整行横幅、列表、图表和工作台。
- AI 组件生成、骨架 slot 填充、积木组合和 AI HTML 都必须带 `gridColumns=12` 与对应 `desktopSpan`；移动端统一单列。
