...
| `trading_account_highlight` | 交易账户重点展示区 | 一个账号的基础信息、收益率、浮动盈亏和 ECharts 盈亏折线图 | `2x2`, `2x1`, `1x2` | 可扩展为宽屏表现图表 | 已纳入白名单 |
...
- 账号详情、更多操作、分页。
- 开户入口应靠近筛选区域或空状态区域。
- 账号卡片必须有清晰层级：账号身份、一个主金额、一个 PnL/风险状态、少量次要字段和操作区；不能把所有指标做成同等权重的彩色小块。
- 如果用户反馈“小卡片里面模块太多、重点太多、排版不行”，先减少卡片内部结构：去掉四宫格指标和多列元信息，只保留主金额、盈亏状态、平台/服务器、保证金或杠杆等 2-3 条安静信息。
