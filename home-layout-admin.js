    ...
    const usedModel = payload.model || config.model;
        ...
        attachRenderModeToConfig(aiConfig, prompt, { renderMode, reason: `${usedProvider.name} fallback` }),
	      ...
	      strategy: normalized.brickTrace?.strategy || normalized.compositionStrategy || "",
		      styleContract: normalized.styleContract?.id || "",
		      goldenStyleContract: normalized.goldenStyleContract?.id || "",
		      sectionTransitions: normalized.sections.map((section) => `${section.id || section.title}:${section.transition || ""}`).filter((item) => !item.endsWith(":")),
